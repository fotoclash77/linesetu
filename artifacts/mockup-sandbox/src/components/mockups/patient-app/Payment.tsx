...
/* ── Default data — used until Booking screen saves its selection ── */
const DEFAULT = {
  tokenType:   'normal' as 'normal' | 'emergency',
  date:        10,
  shiftLabel:  'Morning',
  shiftTime:   '10:00 AM – 2:00 PM',
  shiftClinic: 'HeartCare Clinic',
  shiftLoc:    'Andheri West, Mumbai',
  yourToken:   43,
  payableNow:  20,
  consultFee:  500,
  eAppFee:     10,
...
function readBooking() {
  try {
    const raw = localStorage.getItem('linesetu_booking');
    return raw ? { ...DEFAULT, ...JSON.parse(raw) } : DEFAULT;
  } catch { return DEFAULT; }
}

/* Format date number → e.g. "Fri, 10 Apr 2026" */
const DAY_NAMES = ['Sun','Mon','Tue','Wed','Thu','Fri','Sat'];
function fmtDate(d: number) {
  // April 1 2026 = Wednesday (index 3)
  const dow = (3 + d - 1) % 7;
  return `${DAY_NAMES[dow]}, ${d} Apr 2026`;
}

export function Payment() {
  const [bk, setBk] = useState(readBooking);
  const [method, setMethod] = useState('upi');
  const [upiId,  setUpiId]  = useState('');

  /* Re-read whenever this iframe gains focus (simulates navigation from Booking) */
  useEffect(() => {
    const onFocus = () => setBk(readBooking());
    window.addEventListener('focus', onFocus);
    return () => window.removeEventListener('focus', onFocus);
  }, []);

  const isEmergency = bk.tokenType === 'emergency';
  const tokenLabel  = `${isEmergency ? '#E' : '#'}${bk.yourToken} · ${isEmergency ? 'Emergency' : 'Online'}`;
  ...
  return (
    <div style={{ width: 390, height: 844, background: BG, fontFamily: "'Inter', sans-serif", display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
      ...
      {/* Glow orbs */}
      <div style={{ position: 'absolute', top: -50, left: -50, width: 220, height: 220, borderRadius: '50%',
        background: isEmergency ? 'radial-gradient(circle, rgba(239,68,68,0.2) 0%, transparent 70%)' : 'radial-gradient(circle, rgba(99,102,241,0.22) 0%, transparent 70%)',
        filter: 'blur(24px)', pointerEvents: 'none' }} />
      <div style={{ position: 'absolute', bottom: 160, right: -60, width: 180, height: 180, borderRadius: '50%',
      ...
      <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', paddingBottom: 100 }}>
        ...
        {/* ── TOKEN TYPE BANNER (read-only, from Booking) ── */}
        <div style={{ margin: '0 18px 14px', padding: '12px 14px', borderRadius: 18,
          background: isEmergency ? 'rgba(239,68,68,0.12)' : 'rgba(99,102,241,0.12)',
          border: `1.5px solid ${isEmergency ? 'rgba(239,68,68,0.35)' : 'rgba(99,102,241,0.35)'}`,
          display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 42, height: 42, borderRadius: 13, flexShrink: 0,
            background: isEmergency ? 'rgba(239,68,68,0.22)' : 'rgba(99,102,241,0.22)',
            display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            {isEmergency
              ? <ShieldAlert style={{ width: 20, height: 20, color: '#F87171' }} />
              : <Ticket       style={{ width: 20, height: 20, color: '#A5B4FC' }} />}
          </div>
          <div style={{ flex: 1 }}>
            <div style={{ fontSize: 13, fontWeight: 800, color: isEmergency ? '#F87171' : '#A5B4FC', marginBottom: 2 }}>
              {isEmergency ? 'Emergency Token' : 'Normal Token'} — {tokenLabel}
            </div>
            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.38)', lineHeight: 1.5 }}>
              {isEmergency
                ? 'Priority — placed at the top of the queue · Consult ₹700 at clinic'
                : 'Standard queue order · Consult ₹500 at clinic'}
            </div>
          </div>
          {isEmergency && <Siren style={{ width: 14, height: 14, color: '#F87171', flexShrink: 0 }} />}
        </div>
        ...
        <div style={{ margin: '0 18px 14px', borderRadius: 20, overflow: 'hidden', ...GLASS }}>
          ...
          <div style={{ padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 12, borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>Cardiologist · {bk.shiftClinic}</span>
            </div>
            <div style={{ padding: '3px 9px', borderRadius: 8, flexShrink: 0,
              background: isEmergency ? 'rgba(239,68,68,0.15)' : 'rgba(34,197,94,0.15)',
              border: `1px solid ${isEmergency ? 'rgba(239,68,68,0.35)' : 'rgba(34,197,94,0.3)'}` }}>
              <span style={{ fontSize: 10, fontWeight: 700, color: isEmergency ? '#F87171' : '#4ADE80' }}>
                {isEmergency ? 'Emergency' : 'Confirmed'}
              </span>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 1, background: 'rgba(255,255,255,0.035)' }}>
            {[
              { icon: CalendarCheck,                      label: 'Date',      val: fmtDate(bk.date),      color: '#67E8F9' },
              { icon: Sunrise,                             label: 'Shift',     val: `${bk.shiftLabel} Shift`, color: '#F59E0B' },
              { icon: Clock,                               label: 'Timing',    val: bk.shiftTime,          color: '#818CF8' },
              ...
              <div key={label} style={{ padding: '11px 13px',
          ...
          <div style={{ padding: '9px 14px', display: 'flex', alignItems: 'center', gap: 6, borderTop: '1px solid rgba(255,255,255,0.05)' }}>
            ...
            <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.5)', fontWeight: 600 }}>{bk.shiftClinic}</span>
            <span style={{ color: 'rgba(255,255,255,0.18)' }}>·</span>
            <MapPin style={{ width: 10, height: 10, color: 'rgba(255,255,255,0.3)', flexShrink: 0 }} />
            <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)' }}>{bk.shiftLoc}</span>
          </div>
        </div>
        ...
        <div style={{ margin: '0 18px 14px', borderRadius: 20, overflow: 'hidden', ...GLASS }}>
          ...
          <div style={{ padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                <div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: 'rgba(255,255,255,0.85)' }}>
                    Clinic E-Appointment{isEmergency ? ' (Priority)' : ''}
                  </div>
                  <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.35)' }}>
                    {isEmergency ? 'Emergency priority token fee' : 'Online token booking fee'}
                  </div>
                </div>
              </div>
              <span style={{ fontSize: 14, fontWeight: 800, color: isEmergency ? '#F87171' : '#67E8F9' }}>₹{bk.eAppFee}</span>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <span style={{ fontSize: 14, fontWeight: 800, color: '#818CF8' }}>₹{bk.platformFee}</span>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', borderRadius: 14,
              background: isEmergency
                ...
                : 'linear-gradient(135deg, rgba(79,70,229,0.2), rgba(6,182,212,0.12))',
              ...
              <div>
                ...
                <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.28)' }}>₹{bk.eAppFee} + ₹{bk.platformFee} = ₹{bk.payableNow}</div>
              </div>
              <div style={{ fontSize: 30, fontWeight: 900, lineHeight: 1, letterSpacing: '-0.5px', color: isEmergency ? '#F87171' : '#A5B4FC' }}>
                ₹{bk.payableNow}
              </div>
            </div>
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 6, padding: '8px 10px', borderRadius: 12, background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)' }}>
              ...
              <span style={{ fontSize: 10, color: 'rgba(255,255,255,0.4)', lineHeight: 1.6 }}>
                Consultation fee of{' '}
                <strong style={{ color: isEmergency ? '#F87171' : '#FCD34D' }}>₹{bk.consultFee}</strong>
                {isEmergency ? ' (Emergency)' : ''} is paid directly at the clinic. Not included here.
              </span>
            </div>
          </div>
        </div>
