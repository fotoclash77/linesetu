...
export default function QueueScreen() {
  ...
  const downloadPDF = async () => {
    const realTokens = allSerialRows.filter((r): r is Token => !('gap' in r));
    if (realTokens.length === 0) { Alert.alert('No tokens', 'No tokens to export for this shift.'); return; }
    const docName = doctor ? (doctor.name.startsWith('Dr.') ? doctor.name : `Dr. ${doctor.name}`) : 'Doctor';
    const shiftLabel = shift === 'morning' ? 'Morning' : 'Evening';
    const dateLabel = new Date(schedDate + 'T00:00:00').toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' });
    const clinicLabel = clinicName || 'Clinic';
    const tableRows = realTokens.map((t, i) => {
      const genderStr = t.gender === 'M' || t.gender === 'male' ? 'M'
        : t.gender === 'F' || t.gender === 'female' ? 'F' : (t.gender ?? '—');
      const age   = t.age  ? `${t.age} yr` : '—';
      const area  = t.area ?? '—';
      const phone = t.patientPhone || '—';
      const statusMap: Record<DisplayStatus, string> = { consulting: 'Consulting', waiting: 'Waiting', done: 'Done', skipped: 'Skipped' };
      const status = statusMap[t.displayStatus] ?? t.displayStatus;
      const isE   = t.type === 'emergency';
      const rowBg = i % 2 === 0 ? '#ffffff' : '#f8faff';
      return `<tr style="background:${rowBg};">
        <td style="padding:8px 10px;text-align:center;font-weight:700;font-size:14px;color:${isE ? '#dc2626' : '#1e3a5f'};">${isE ? 'E' : '#'}${String(t.tokenNumber).padStart(2, '0')}</td>
        <td style="padding:8px 10px;font-weight:600;font-size:13px;color:#0f172a;">${t.patientName}</td>
        <td style="padding:8px 10px;text-align:center;font-size:12px;color:#475569;">${age}</td>
        <td style="padding:8px 10px;text-align:center;font-size:12px;color:#475569;">${genderStr}</td>
        <td style="padding:8px 10px;font-size:12px;color:#475569;">${area}</td>
        <td style="padding:8px 10px;font-size:12px;color:#475569;font-family:monospace;">${phone}</td>
        <td style="padding:8px 10px;text-align:center;"><span style="font-size:10px;font-weight:700;padding:2px 7px;border-radius:4px;background:${t.displayStatus==='done'?'#dcfce7':t.displayStatus==='consulting'?'#fef3c7':t.displayStatus==='skipped'?'#f3e8ff':'#dbeafe'};color:${t.displayStatus==='done'?'#15803d':t.displayStatus==='consulting'?'#b45309':t.displayStatus==='skipped'?'#7c3aed':'#1d4ed8'};">${status}</span></td>
      </tr>`;
    }).join('');
    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8"/>
    <style>
      @page{size:A4;margin:18mm 14mm;}
      body{font-family:-apple-system,Arial,sans-serif;color:#0f172a;}
      h1{font-size:20px;font-weight:800;margin:0 0 2px;color:#1e3a5f;}
      .sub{font-size:12px;color:#64748b;margin-bottom:18px;}
      table{width:100%;border-collapse:collapse;font-size:13px;}
      thead tr{background:#1e3a5f;color:#fff;}
      thead th{padding:9px 10px;text-align:left;font-size:11px;font-weight:700;letter-spacing:0.5px;text-transform:uppercase;}
      thead th:first-child,thead th:nth-child(3),thead th:nth-child(4),thead th:last-child{text-align:center;}
      td{border-bottom:1px solid #e5e7eb;}
      .footer{margin-top:20px;font-size:10px;color:#94a3b8;text-align:center;}
    </style></head><body>
    <h1>Token List — ${docName}</h1>
    <div class="sub">${clinicLabel} · ${shiftLabel} Shift · ${dateLabel} · ${realTokens.length} token${realTokens.length !== 1 ? 's' : ''}</div>
    <table><thead><tr>
      <th style="width:60px">Token</th><th>Patient Name</th><th style="width:58px">Age</th>
      <th style="width:40px">Sex</th><th>Address / Area</th><th>Phone</th><th style="width:75px">Status</th>
    </tr></thead><tbody>${tableRows}</tbody></table>
    <div class="footer">Generated by LineSetu · ${new Date().toLocaleString('en-IN')}</div>
    </body></html>`;
    setPdfBusy(true);
    try {
      if (Platform.OS === 'web') {
        ...
        if (canShare) {
          await Sharing.shareAsync(uri, { mimeType: 'application/pdf', dialogTitle: 'Save Token List' });
        } else {
                ...
                {tab === 'all' && all.length > 0 && (
                  <TouchableOpacity
                    ...
                    style={{
                      ...
                      backgroundColor: 'rgba(99,102,241,0.15)', borderWidth: 1, borderColor: 'rgba(129,140,248,0.35)',
                      opacity: pdfBusy ? 0.5 : 1,
                    }}
                  >
                    {pdfBusy
                      ? <ActivityIndicator size="small" color="#A5B4FC" />
                      : <Feather name="download" size={13} color="#A5B4FC" />}
                    <Text style={{ fontSize: 11, fontWeight: '800', color: '#A5B4FC' }}>PDF</Text>
                  </TouchableOpacity>
                )}
              </View>
...
const S = StyleSheet.create({
  ...
  // Waiting list header
  waitingHeader:{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, paddingTop: 6, borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.07)' },
